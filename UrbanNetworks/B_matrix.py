...
def plotMatrix(o_mat, **kwargs):
	...
	bb_cm = pylab.get_cmap("jet")
	# init colormap such that its members are available
	bb_cm._init()
	# set the first value to black
	bb_cm._lut[0,: ] = (1,1,1,1)
	H = pylab.imshow( mat, cmap=bb_cm,**kwargs)
	pylab.axis('tight')	
...
if __name__ == '__main__':
	try:
		...
		G = networkx.read_edgelist(fin) #, delimiter="\t")
	except:
		print "could not read edgelist file"
